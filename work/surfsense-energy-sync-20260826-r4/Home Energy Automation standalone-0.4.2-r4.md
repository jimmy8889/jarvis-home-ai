...
The primary whole-site load source is the SAJ signed-watt
`BackupTotalLoadPowerWatt` register at `0x40AB` because the entire site is on the
backup circuit. Five-minute ordinary-house outcomes subtract measured Tesla and
hot-water demand once; raw backup load remains separately visible. The dashboard
