...
class IntegrationSettings:
    ...
    energy_manager_url: str = ""
    energy_manager_timeout_seconds: float = 2.0
    energy_manager_snapshot_interval_seconds: float = 2.0
    energy_manager_plan_interval_seconds: float = 20.0
    energy_manager_stale_after_seconds: float = 10.0
    ...
    office_sim_rig_switch_entity_id: str = ""
    codex_usage_used_entity_id: str = ""
    codex_usage_remaining_entity_id: str = ""
    codex_usage_reset_entity_id: str = ""
...
def load_settings(path: str | Path) -> Settings:
    ...
    integrations = IntegrationSettings(
        ...
        energy_manager_url=str(
            integration_values.get("energy_manager_url", "")
        ).rstrip("/"),
        energy_manager_timeout_seconds=float(
            integration_values.get("energy_manager_timeout_seconds", 2)
        ),
        energy_manager_snapshot_interval_seconds=float(
            integration_values.get("energy_manager_snapshot_interval_seconds", 2)
        ),
        energy_manager_plan_interval_seconds=float(
            integration_values.get("energy_manager_plan_interval_seconds", 20)
        ),
        energy_manager_stale_after_seconds=float(
            integration_values.get("energy_manager_stale_after_seconds", 10)
        ),
        ...
        office_sim_rig_switch_entity_id=str(
            integration_values.get("office_sim_rig_switch_entity_id", "")
        ).strip(),
        codex_usage_used_entity_id=str(
            integration_values.get("codex_usage_used_entity_id", "")
        ).strip(),
        codex_usage_remaining_entity_id=str(
            integration_values.get("codex_usage_remaining_entity_id", "")
        ).strip(),
        codex_usage_reset_entity_id=str(
            integration_values.get("codex_usage_reset_entity_id", "")
        ).strip(),
        teslamate_adapter_url=str(
    ...
    if integrations.energy_manager_url:
        energy_manager_url = urlparse(integrations.energy_manager_url)
        if (
            energy_manager_url.scheme not in {"http", "https"}
            or not energy_manager_url.netloc
            or energy_manager_url.username is not None
            or energy_manager_url.password is not None
            or energy_manager_url.path not in {"", "/"}
            or energy_manager_url.params
            or energy_manager_url.query
            or energy_manager_url.fragment
        ):
            raise ValueError(
                "integrations.energy_manager_url must be an HTTP(S) origin"
            )
    if not 0.2 <= integrations.energy_manager_timeout_seconds <= 30:
        raise ValueError(
            "integrations.energy_manager_timeout_seconds must be between 0.2 and 30"
        )
    if not 0.5 <= integrations.energy_manager_snapshot_interval_seconds <= 30:
        raise ValueError(
            "integrations.energy_manager_snapshot_interval_seconds must be between 0.5 and 30"
        )
    if not 2 <= integrations.energy_manager_plan_interval_seconds <= 300:
        raise ValueError(
            "integrations.energy_manager_plan_interval_seconds must be between 2 and 300"
        )
    if not (
        integrations.energy_manager_snapshot_interval_seconds
        <= integrations.energy_manager_stale_after_seconds
        <= 120
    ):
        raise ValueError(
            "integrations.energy_manager_stale_after_seconds must be between "
            "the snapshot interval and 120"
        )
    ...
    for setting_name, entity_id in (
        ...
        ("codex_usage_used_entity_id", integrations.codex_usage_used_entity_id),
        ("codex_usage_remaining_entity_id", integrations.codex_usage_remaining_entity_id),
        ("codex_usage_reset_entity_id", integrations.codex_usage_reset_entity_id),
    ...
    if (
        integrations.office_sim_rig_switch_entity_id
        and not integrations.office_sim_rig_switch_entity_id.startswith("switch.")
    ):
        raise ValueError(
            "integrations.office_sim_rig_switch_entity_id must be a switch entity"
        )
    for setting_name, entity_id in (
