...
- The page reads Pilot Core's unauthenticated readiness projection. No Pilot
  administrator credential is installed on the node.
...
Room-specific data and controls remain deferred until the panel has a logical
room assignment and a device-scoped Pilot Core credential.
...
The role validates Debian 13, ARM64, and Raspberry Pi hardware before making
changes. A pending display-mode reboot is recorded on disk, so an interrupted
playbook resumes safely.
