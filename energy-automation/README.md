...
Standalone release `0.4.20` exposes **Energy automation** and **Hot water
enabled** as the first controls on both dashboards. Disabling Energy automation
performs one self-consumption handover and then sends no more inverter commands.
EV and forecast dispatch are ignored, while the independent hot-water governor
runs a strict 11:00–14:00 Brisbane timer. Disabling Hot water enabled is Away
mode and always forces the relay off, overriding the timer and every plan. Both
settings persist in SQLite and round-trip through
`input_boolean.energy_manager_control_enabled` and
`input_boolean.energy_manager_hot_water_enabled` in Home Assistant.
The fixed-clock governor is deliberately outside the SAJ dispatch path. It can
therefore maintain the relay window during an Amber, forecast, Modbus, MQTT,
InfluxDB or Home Assistant outage. With invalid SAJ telemetry it reports source
control unavailable: daily service remains guaranteed, but battery-versus-grid
isolation cannot be enforced or claimed until Modbus recovers.

Hot-water rolling plans include the active five-minute interval even when the
planner runs just after the Amber boundary. An active element keeps its current
block committed; routine replanning must not continually defer or chatter an
already-due service. Solar qualification, confirmed runtime and the 4pm rescue
deadline continue to govern physical operation.
Manager startup establishes a five-second relay-off baseline before resuming a
due hot-water block, so upgrades and failover cannot leave an operating element
stuck in an unconfirmed, non-accruing state.
The internal and Home Assistant dashboards write validated manager settings
directly over the trusted LAN without an API token. Direct actuator overrides
remain authenticated.
Tesla planning and live dispatch use the same configured 0.69 kW per
three-phase amp conversion. The minimum 6 A block is therefore 4.14 kW in both
paths and remains actionable at the opportunistic FIT threshold.
...
Following the site rewiring, the primary whole-site load source is SAJ signed
`TotalLoadPower`; nothing is presently connected to the backup output.
`BackupTotalLoadPowerWatt` at `0x40AB` remains visible as diagnostics only and
cannot drive planning. Five-minute ordinary-house outcomes subtract measured
Tesla and hot-water demand once. The dashboard
label **Normal house total** includes Server + desk. Server + desk is a submeter
annotation inside that total, not a separately conserved load and not a
deduction from it. The dedicated SAJ battery-power
...
### 2026-08-26 timeline and dashboard efficiency correction

- Added separate **Dispatch plan** and **What happened** tabs to the canonical
  manager timeline embedded by Home Assistant. Actual history is the most recent
  288 completed five-minute outcomes and is fetched only while selected.
- Corrected hot-water chart bars to stop at the element's physical 3.7 kW.
- Stopped rebuilding full plan and Amber SVGs on every two-second snapshot;
  semantic plan-ID changes now trigger those renders, history refreshes at most
  once per minute, and hidden pages close their event stream.
- Confirmed the direct reader and HA SAJ integration both use register `0x40AB`.
  Corrected outcome accounting that had labelled the whole backup load as
  ordinary house demand without removing measured Tesla and hot water.
...
- The cutover-era immutable release `standalone-0.2.9` added semantic command identities,
  ...
  requires FIT to beat direct wear plus uncertainty. Import avoidance is
  protected by the forecast SOC trajectory; it is not charged a second time
  against energy already proven surplus to that trajectory.
...
marginal wear allowance plus $0.02/kWh uncertainty, and the protected SOC
trajectory. Energy above that trajectory is not additionally charged with the
live import price because doing so would double-count retention. The active
floor is the greater of the dashboard value and direct export costs. For example, a
...
- With a valid pair, household supply remains a physical priority regardless of
  ...
  against live FIT, the editable sell floor, direct wear/uncertainty, and the
  future protected reserve.
...
### 2026-08-26 protected-surplus sell-floor correction

- Made the dashboard minimum sell price an effective battery-export trigger
  for energy above the forecast protected SOC trajectory.
- Removed the duplicate live import-avoidance charge from that surplus tranche;
  direct 8 c/kWh wear and 2 c/kWh uncertainty remain mandatory.
- The protected reserve remains an independent hard dispatch gate, so meeting
  the sell floor cannot consume energy allocated to overnight house demand.
- Physically verified the SAJ forced-discharge schedule register against the
  dedicated site meter. It controls export beyond local demand; it is not the
  total battery-terminal power. Profitable dispatch now writes the planned
  site-export target directly, while the battery target remains the expected
  battery output used for planning and display. This removes systematic export
  overshoot equal to the contemporaneous house load.
- Added retained MQTT and Home Assistant Discovery publication for the Tesla
  BLE controller's physical `James Car Home` presence and ultrasonic `Garage
  Preset` states, including a combined source-labelled JSON payload.

### 2026-08-26 Home Assistant Tesla comfort and access proxy

- Kept the standalone manager as the sole client of the Tesla ESPHome native
  API; the disabled Home Assistant ESPHome config entry remains disabled.
- Added a local manager vehicle API and a Home Assistant proxy integration for
  cabin climate and temperature, vehicle lock/unlock, charge-port latch,
  charge-port door and cable unlock, window vent/close, steering-wheel heat,
  defrost, wake, flash and horn.
- Vehicle commands are rejected unless the car is physically home and local
  BLE is healthy. The public dashboard proxy overwrites its client-IP header;
  only the configured Home Assistant host or a valid bearer token can send a
  command. MQTT remains state-only and anonymous clients cannot actuate the
  vehicle.
- Seat heating/cooling remains intentionally absent because the pinned and
  current upstream Tesla BLE packages expose no seat action. No cosmetic or
  unconfirmed seat control is published.

### 2026-08-26 Tesla opportunistic charging stability

- Corrected the live Tesla governor to use the configured 0.69 kW/A three-phase
  conversion everywhere. A planned 4.14 kW block now remains a valid 6 A
  command instead of being rounded down to 5 A and switched off.
- During a scheduled opportunistic block, the rolling plan is the stable
  display forecast. During positive low FIT the live charging target comes
  directly from fresh Ecowitt expected uncurtailed generation, less non-EV
  demand and the configured attenuation.
- For negative FIT specifically, the opportunity-cost attenuation is removed:
  the allowance is fresh Ecowitt expected PV minus house and hot-water demand.
  Positive low-FIT charging retains the configured conservative attenuation.
- Applied a 0.3 kW deadband to the two-second grid meter so harmless zero-point
  flicker does not chatter a minimum-current Tesla session.
- An active Tesla now reduces by one amp per 30-second interval, holds 6 A for
  one interval, then stops cleanly if the solar shortfall persists.
- Tesla stop now sends both the custom firmware lease cancellation and the
  direct local BLE charger-switch-off command. Fleet control remains disabled.
- The downward ramp now tracks its own last commanded current. Noisy BLE
  feedback cannot repeatedly reset the 6 A hold and deadlock the final stop.
- An off charger is confirmed from its local switch state; its dormant amp
  setting no longer generates repeated stop calls or delays hot-water start.
- Hot-water confirmation no longer switches the relay off merely because EV,
  price, plan or PV controls recalculate during the 45-second element test.
- Hot water now waits until the BLE-reported Tesla switch/current has matched
  its command for 30 seconds before beginning the element load-step test. EV
  ramping can no longer create a false 3.7 kW actuator mismatch.
- If the Tesla is already charging and proven PV/export headroom can carry the
  element, its current is pinned to the physical BLE value for confirmation so
  a moving calculated target cannot delay hot water.
