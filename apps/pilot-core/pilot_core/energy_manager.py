...
def _direction(
    ...
    negative: str,
    *,
    idle_watts: float = POWER_IDLE_WATTS,
) -> str:
    if value is None or abs(value) < idle_watts:
        return "idle"
...
def _has_flow_edge(
    ...
    *,
    ...
    """Mirror the standalone dashboard's edge-activity test."""

    return any(
        isinstance(edge, dict)
        and (source is None or edge.get("from") == source)
        and (sink is None or edge.get("to") == sink)
        and (_number(edge.get("kw")) or 0.0) > 0.02
        for edge in edges
    )


def _manager_live_power(snapshot: dict[str, Any]) -> dict[str, Any]:
    """Project the manager's own live-flow semantics without reinterpreting it.

    The standalone dashboard treats a flow graph with an edge list as the
    canonical live contract. Raw inverter telemetry is only its legacy
    fallback. Pilot must make the same choice or the two surfaces can show
    different values from the same snapshot.
    """

    telemetry = _mapping(snapshot.get("telemetry"))
    devices = _mapping(snapshot.get("devices"))
    graph = _mapping(snapshot.get("power_flow")) or _mapping(snapshot.get("flow"))
    raw_edges = graph.get("edges")
    if isinstance(raw_edges, list):
        edges = raw_edges
        sources = _mapping(graph.get("sources_kw"))
        sinks = _mapping(graph.get("sinks_kw"))
        submeters = _mapping(graph.get("submeters_kw"))
        source_battery = _number(sources.get("battery")) or 0.0
        sink_battery = _number(sinks.get("battery")) or 0.0
        source_grid = _number(sources.get("grid")) or 0.0
        sink_grid = _number(sinks.get("grid")) or 0.0
        site_load_kw = _number(graph.get("site_load_kw"))
        if site_load_kw is None:
            site_load_kw = sum(
                _number(sinks.get(name)) or 0.0
                for name in ("house", "hot_water", "ev")
            )
        rack_kw = _number(submeters.get("server_rack"))
        if rack_kw is None:
            rack_kw = _number(sinks.get("server_rack"))
        return {
            "solar_w": _watts(sources.get("solar")),
            "grid_w": _watts(source_grid - sink_grid),
            "battery_w": _watts(source_battery - sink_battery),
            "home_load_w": _watts(site_load_kw),
            "server_rack_w": _watts(rack_kw),
            "vehicle_w": _watts(_number(sinks.get("ev")) or 0.0),
            "hot_water_w": _watts(_number(sinks.get("hot_water")) or 0.0),
            "flow_active": {
                "solar": _has_flow_edge(edges, source="solar"),
                "grid": _has_flow_edge(edges, source="grid")
                or _has_flow_edge(edges, sink="grid"),
                "battery": _has_flow_edge(edges, source="battery")
                or _has_flow_edge(edges, sink="battery"),
                "home": _has_flow_edge(edges, sink="house"),
                "server_rack": rack_kw is not None and rack_kw > 0.02,
                "vehicle": _has_flow_edge(edges, sink="ev"),
                "hot_water": _has_flow_edge(edges, sink="hot_water"),
            },
            "direction_deadband_w": 50.0,
            "device_deadband_w": 30.0,
            "source": "power_flow",
        }
    ...
    return {
        ...
        "direction_deadband_w": POWER_IDLE_WATTS,
        "device_deadband_w": POWER_IDLE_WATTS,
...
class EnergyManagerClient:
    ...
    def __init__(
        ...
        *,
        ...
        self._daily_rows: list[dict[str, Any]] | None = None
        ...
        self._series_error: str | None = None
    ...
    async def poll_once(self, *, plan_due: bool = False) -> bool:
        ...
        async with self._refresh_lock:
            ...
            if plan_due or plan_changed:
                plan_refreshed = await self._refresh_plan()
    ...
    async def _request_list(self, path: str) -> list[dict[str, Any]]:
        ...
        if not isinstance(payload, list) or not all(
            ...
            raise ValueError("response is not an array of objects")
        return payload
    ...
    def _record_history(
        ...
        if self._history_samples:
            ...
            if previous is not None and (observed_at - previous).total_seconds() < 10:
                return
        live_power = _manager_live_power(payload)
        self._history_samples.append(
            {
                "at": observed_at.isoformat(),
                "solar_w": live_power["solar_w"],
                "home_load_w": live_power["home_load_w"],
                "battery_w": live_power["battery_w"],
                "vehicle_w": live_power["vehicle_w"],
            }
        )
    ...
    async def _refresh_dashboard_data(self) -> None:
        """Refresh the manager's authoritative journal-backed display data."""

        try:
            self._series = deepcopy(await self._request_list("/api/v1/series"))
            self._series_error = None
        except (httpx.HTTPError, ValueError) as error:
            self._series_error = str(error)[:300]
        try:
            self._daily_rows = deepcopy(await self._request_list("/api/v1/daily"))
            self._daily_error = None
        except (httpx.HTTPError, ValueError) as error:
            self._daily_error = str(error)[:300]
    ...
    def health(self) -> dict[str, Any]:
        ...
        return {
            ...
            "series_error": self._series_error,
            "daily_error": self._daily_error,
        }
    ...
    def dashboard_fields(self) -> dict[str, Any]:
        ...
        fields["daily"] = self.daily_projection()
        return fields

    def daily_projection(self) -> dict[str, Any]:
        local_date = self.clock().astimezone(
            ZoneInfo(self.settings.home_timezone)
        ).date().isoformat()
        snapshot_daily = _mapping(
            _mapping(self._snapshot).get("daily_energy")
        )
        if snapshot_daily.get("local_date") == local_date:
            return {
                "solar_generated_kwh": _number(
                    snapshot_daily.get("solar_generated_kwh")
                ),
                "home_used_kwh": _number(
                    snapshot_daily.get("house_consumed_kwh")
                ),
                "grid_exported_kwh": _number(
                    snapshot_daily.get("grid_exported_kwh")
                ),
            }
        row = next(
            (
                item
                for item in self._daily_rows or []
                if item.get("local_date") == local_date
            ),
            {},
        )
        return {
            "solar_generated_kwh": _number(row.get("pv_kwh")),
            "home_used_kwh": _number(
                row.get("whole_house_load_kwh")
                if row.get("whole_house_load_kwh") is not None
                else row.get("house_load_kwh")
            ),
            "grid_exported_kwh": _number(row.get("export_kwh")),
        }
    ...
    def history_projection(self) -> dict[str, Any]:
        ...
        journal_samples = [
            {
                "at": item.get("interval_start"),
                "solar_w": _watts(item.get("average_pv_kw")),
                "home_load_w": _watts(item.get("average_whole_house_load_kw")),
                "battery_w": _watts(item.get("average_battery_kw")),
                "vehicle_w": _watts(item.get("average_ev_kw")),
            }
            for item in self._series or []
        ]
        source_samples = journal_samples or self._history_samples
        samples = [
            sample
        ...
        return {
            ...
            "source": (
                "standalone_energy_manager_series"
                if journal_samples
                else "standalone_energy_manager_cache"
            ),
            "series": [
                {
                    "id": identifier,
    ...
    def _projection(self, *, include_plan_points: bool) -> dict[str, Any]:
        ...
        live_power = _manager_live_power(snapshot)
        solar_w = live_power["solar_w"]
        grid_w = live_power["grid_w"]
        battery_w = live_power["battery_w"]
        home_w = live_power["home_load_w"]
        soc = _number(telemetry.get("soc_pct"))
        vehicle_w = live_power["vehicle_w"]
        hot_water_w = live_power["hot_water_w"]
        ...
        server_w = live_power["server_rack_w"]
        ...
        directions = {
            "grid": _direction(
                ...
                "importing",
                "exporting",
                idle_watts=live_power["direction_deadband_w"],
            ),
            "battery": _direction(
                battery_w,
                "discharging",
            ...
            "vehicle": (
                "charging"
                if vehicle_w is not None
                and vehicle_w >= live_power["device_deadband_w"]
                else "idle"
            ),
            "hot_water": (
                "heating"
                if hot_water_w is not None
                and hot_water_w >= live_power["device_deadband_w"]
                else "idle"
            ),
            "server_rack": "consuming" if server_w is not None and server_w >= 20 else "idle",
        }
        power = {
            "solar_w": solar_w,
            "grid_w": grid_w,
            ...
            "flow_active": live_power["flow_active"],
            "source": live_power["source"],
        }
